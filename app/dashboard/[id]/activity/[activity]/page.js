...
const VisualEditor = () => {
  ...
  function getFullSelector(element) {
    ...
    while (element && element.nodeType === 1) {
      // Ensure the element is an element node
      ...
      if (element.id) {
        ...
        const filteredClasses = [...element.classList].filter(
          (className) => !className.startsWith("prezy-")
        );
  ...
  useEffect(() => {
    ...
    const projectId = window.location.pathname
      .split("/activity/")[0]
      .split("dashboard/")[1]; // Assuming the projectId is at index 1
    const activityId = window.location.pathname.split("/activity/")[1]; // Assuming the activityId is at index 3
  ...
  const handleDrop = (event) => {
    ...
    if (draggedHTMl && event.target.id.includes("personalized-element")) {
      ...
      if (draggedHTMl === "draggable-text") {
        ...
        const targetEle = iframeRef.current.contentDocument.body.querySelector(
          "#" + event.target.id
        );
  ...
  const loadWebsite = async (url) => {
    ...
    try {
      const projectId = window.location.pathname
        .split("/activity/")[0]
        .split("dashboard/")[1]; // Extract projectId
      ...
      if (data) {
        fetch(
          ...
          }/api/proxy?url=${encodeURIComponent(url)}&jsPath=${
            data.jsFilePath.split("/public")[1]
          }`
        )
          .then((res) => {
            if (!res.ok) {
  ...
  const handleSaveText = (newText) => {
    ...
    if (currentElement) {
      ...
      newChange = `Changed text of element with tag ${getFullSelector(
        currentElement
      ).slice(0, 70)}... to <p>${newText}</p>`;

      setDbChanges((prevHistory) => [
        ...prevHistory,
        {
          type: "modified",
          selector: getFullSelector(currentElement).replace(
            /(?<=\bhtml\b)(\.[a-zA-Z0-9\-_]+)*|(?<=\bbody\b)(\.[a-zA-Z0-9\-_]+)*/g,
            ""
          ),
          newText: newText,
        },
  ...
  const handleMenuAction = (action) => {
    ...
    switch (action) {
      ...
      case "remove-element":
        if (element) {
          element.remove();
          newChange = `Removed element with tag ${getFullSelector(
            element
          ).replace(
            /(?<=\bhtml\b)(\.[a-zA-Z0-9\-_]+)*|(?<=\bbody\b)(\.[a-zA-Z0-9\-_]+)*/g,
            ""
      ...
      case "insert-before":
        if (element) {
          ...
          newElement.id =
            "personalized-element-" + generateDateTime4DigitCode();
    ...
    return (
      ...
      >
        <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
          <li
            onClick={() => handleMenuAction("edit-text")}
            style={{ padding: "8px", cursor: "pointer" }}
          >
            Edit Text
          </li>
          <li
            onClick={() => handleMenuAction("remove-element")}
            style={{ padding: "8px", cursor: "pointer" }}
          >
            Remove Element
          </li>
          ...
          {isImage && (
            <>
              <li
                onClick={() => handleMenuAction("remove-image")}
                style={{ padding: "8px", cursor: "pointer" }}
              >
                Remove Image
              </li>
              <li
                onClick={() => handleMenuAction("replace-image")}
                style={{ padding: "8px", cursor: "pointer" }}
              >
                Replace Image
              </li>
  ...
  const removePer = (change) => {
    ...
    const activityId = window.location.pathname.split("/activity/")[1]; // Assuming the activityId is at index 3
    if (
      change?.type == "html-modified" ||
      change?.type == "modal-added" ||
      change?.type == "text-editor" ||
    ...
    <>
      ...
      {activity && (
        <div className="w-full bg-[#333333] py-[10px] px-[20px] flex flex-row items-center justify-between sticky top-0 z-10 ">
          <div className="w-[30%]">
            <h2
              className="text-[#ffffff] text-[22px] font-bold"
              title={activity.activityName}
            >
              {activity.activityType.toUpperCase()} :: {activity.activityName}
            </h2>
          </div>
      ...
      {activity && (
        <div className="flex h-screen">
            ...
            <div className="w-1/4 p-4 bg-gray-100 shadow-xl rounded-lg h-screen ">
              <h3 className="mt-4 text-lg font-semibold relative">
                ...
                <span
                  ...
                  onClick={(e) => setActivityBar(!activityBar)}
                >
                  <X />
                </span>
              </h3>
              <ul className="mt-2 h-[70vh] overflow-y-scroll overflow-x-hidden">
                {activity?.htmlCode.map((change, index) => (
                  <li
                    key={change._id}
                    data-changeId={change._id}
                    className="mb-2 text-[13px] leading-5 px-2 bg-[#ffffff] py-2 rounded rounded-lg border-2 border-gray-400 relative overflow-hidden"
                  >
                    {loading && (
                      <div className="w-full absolute bg-[#f5f5f5] h-full z-10 left-0 top-0 bg-opacity-75 flex items-center justify-center flex-row">
                        {" "}
                        <ClipLoader size={30} color="#888888" />
                      </div>
                    )}
                    <h3 className="font-bold mb-2">{change?.type}</h3>
                    <hr />
                    <p className="m-2" title={change?.selector}>
                      {change?.selector.substring(0, 70) + "..."}
                    </p>
                    <hr />
          ...
          <Dialog open={openModal} setOpen={setOpenModal} className="z-[5]">
            <ModalBox
              open={openModal}
